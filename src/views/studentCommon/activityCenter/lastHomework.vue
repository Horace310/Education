  ...
  <div id="release">
    <div class="main">
      <div class="formwork">
        ...
        <h3 class="title" v-if="state==2">未答题</h3>
      ...
      <div class="ratelist bf bx" v-if="state==0">
...
export default {
  ...
  created() {
    this.queryStudentLastHomework(); //查询上次作业详情
    this.$store.state.stuworknav = "1"; //头部导航
  },
  methods: {
    //点击到对应题目
    showDetails(num) {
    ...
    //查询上次作业详情
    queryStudentLastHomework() {
      var that = this;
      this.$post("/likework/queryStudentLastHomework.do").then(response => {
        //console.log(response)
        if (response.code == "5000") {
          var property = [];
          var str1 = response.data.homework.solveProblem;
          var str2 = response.data.homework.knowledge;
          that.knowledge = str2.replace("\\n", " \n ");
          that.solveProblem = str1.replace("\\n", " \n ");
          that.homework = response.data.homework;
          ...
          if (response.data.ratelist) {
            for (let i = 0; i < response.data.ratelist.length; i++) {
              that.myChartData1.value.push(
                parseInt(response.data.ratelist[i].classRate)
              );
              that.myChartData1.value1.push(
                parseInt(response.data.ratelist[i].studentRate)
              );
              that.myChartData1.name.push("第" + parseInt(i + 1) + "题");
            }
          }
