  ...
  <div class="knowledgesq">
    <div class="cons nowork bf">
      <div class="sec-con flexbox flexrow">
        ...
        <div class="sec">
          <span class="name ft16">关键字：</span>
          <el-input
            @blur.prevent="keyhomework"
            placeholder="请输入关键词搜索未完成作业"
            v-model="keyword"
            clearable
          ></el-input>
        </div>
        <div class="sec">
          <el-button
            type="primary"
            style="background-color:#f25c29;border:none"
            @click="chooseWork"
          >查询</el-button>
        </div>
      ...
      <div
        class="con flexbox cp"
        v-for="(item,index) in hwlist"
        @click="homeworkDetails(item.id,item.flag )"
        :key="index"
      >
        ...
        <p class="flex" style="margin:4px 0 0  50px;" v-if="item.isCollect==0">
          <el-button
            ...
            type="primary"
            style="background-color:#f25c29;border:none"
            v-on:click.stop.prevent="collectHomework(item.id)"
          >收藏</el-button>
        </p>
        <p class="flex" style="margin:4px 0 0  50px;" v-if="item.isCollect==1">
          <el-button
            ...
            type="primary"
            style="background-color:#f25c29;border:none"
            v-on:click.stop.prevent="cancleCollectHomework(item.id)"
          >取消收藏</el-button>
        </p>
...
export default {
  ...
  created() {
    this.$store.state.stuadminnav = "3";
    //查询加入班级下拉列表
