          ...
          <table>
            ...
            <tbody>
              <tr
                ...
                class="trHover"
                @click="findChapterSimilarity(item.id)"
              >
...
export default {
  ...
  methods: {
    ...
    findErrorStamp() {
      let data = { subjectId: this.SubjectId, textbookDetailId: this.pids };
      this.$get("/user/homework/answer/findErrorStamp", data).then(res => {
        if (res.code == "5000") {
          this.errorStamp = res.data;
        }
      });
    }, //查询有错误的章节
    findChapterSimilarity(id) {
      let data = { id: id };
