<template >
  <div class="homeworkDetail" style="background:#fff">
    <goBack :direction="true" link="/homeCommon/workCenter"></goBack>
    ...
    <div style="text-align:center;padding-bottom:20px">
      <el-button class="release" v-if="state==1?true:false" @click="showBox=true">发布</el-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    seeSpare(value) {
      ...
      this.$get("user/homework/question/findSameById", { id: value }).then(
        res => {
          if (res.code == "5000") {
    ...
    addExercises(value) {
      this.$router.push({
        path: "/homeCommon/workCenter/addSpare",
        query: { id: value.id }
      });
      // console.log(value);
      // this.$post("/user/research/question/insertSameQuestion", {
      //   sameId: value.id,
      ...
      // }).then(res => {
      //   if (res.code == "5000") {
      //     this.$message.success("添加备用练习题成功");
      //   }
      // });
    },
    questions(form, e) {
    ...
    getTeam(e) {
      ...
      this.$post("/user/group", data).then(res => {
        res.data.children.unshift({ name: "全部", id: 0 });
    ...
    deleteSpare(value) {
      this.$post("/user/homework/question/deleteSameQuestion", {
        id: value.id
      }).then(res => {
        if (res.code == 5000) {
