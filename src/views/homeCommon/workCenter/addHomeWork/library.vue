                  ...
                  <div class="foot">
                    ...
                    <p class="_right">
                      <span
                        ...
                        style="margin-right:2px;background:blue"
                      >+收藏</span>
            ...
            <div class="example" v-for="(item, index) in problem" :key="index">
              ...
              <div class="analysis">
                  ...
                  <div class="input">
                    <div class="contentTitle">
                      <ul style="display:flex;">
                        <li style="width:50px;padding-left:10px">
                        ...
                        <el-input style="flex:1" v-model="item.jiexi" placeholder="文本解析"></el-input>
                      </ul>
                    </div>
                    <div
                      :id="'content' + index"
                ...
                <div class="item">
                  ...
                  <div class="input">
                    <el-select v-model="item.questionScore" placeholder="请选择分数">
  ...
  methods: {
    ...
    UploadImages(e) {
      ...
      this.uploading(e, res => {
        let strIndex = res.res.requestUrls[0].lastIndexOf("?");
        let str = res.res.requestUrls[0].substring(0, strIndex);
        this.resolution = "<div><img src=" + str + "></div>";
        console.log(this.resolution);
        document.getElementById(
          "content" + index
        ).innerHTML = `<div><img src="${str}" /></div>`;
      });
      return false;
    ...
    confirm(item, index) {
      let data = {
        ...
        answer: item.answer,
        resolution: this.resolution ? this.resolution : "" + item.jiexi,
    ...
    queryHomework(e) {
      let data = {
        ...
        rows: this.pageSize,
        pid: sessionStorage.getItem("sectionId")
      };
      Utils.$emit("bookId", this.textbookId);
      this.$get("/user/homework/store/findAllList", data).then(res => {
        if (res.code == "5000") {
          this.tiku = res.data.date;
          // for (let i of this.tiku) {
          //   if (!i.typeName) {
          //     console.log(i.typeId);
          //     console.log(this.QuestionType);
          //     console.log(this.QuestionType.find(item => i.typeId == item.id));
          //     i.typeName = this.QuestionType.find(
          //       item => i.typeId == item.id
          //     ).name;
          //   }
          // }
