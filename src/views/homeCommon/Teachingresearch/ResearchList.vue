    ...
    <div class="navTop">
      <div class="classBox">
        <div class="heard">
          ...
          <goBack :direction="true" link="/homeCommon/TeachingResearch"></goBack>
        </div>
        <div class="msgBox">
          <div class="msgList">
            <span>教研组名称：</span>
            ...
            <div class="GroupingList" v-for="(item, index) in studentListItem" :key="index">
              <div class="GroupingLeft">
                <div>
                  ...
                  <span
                    style="padding:5px;background-color:#ccece7;color:#56ac9e;"
...
export default {
  ...
  methods: {
    ...
    //根据ID获取班级详情信息
    getDetailByid() {
      this.$post("/user/research/queryById", { id: this.$route.query.id }).then(
        res => {
          if (res.code == "5000") {
    ...
    addteam() {
      this.$prompt("请输入分组名", "提示", {
        ...
        .then(({ value }) => {
          let data = { rId: this.$route.query.id, name: value };
    ...
    changeClassName() {
      this.$prompt("请输入修改教研组名称", "提示", {
        ...
        .then(({ value }) => {
          let data = { name: value, id: this.$route.query.id };
