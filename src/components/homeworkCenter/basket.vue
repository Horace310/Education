      ...
      <table>
        ...
        <tbody>
          ...
          >
            ...
            <td style="padding:10px;font-size:14px">{{item.typeNameTwo||item.typeName||item.type }}</td>
  ...
  methods: {
    ...
    releases() {
      ...
      if (this.homeworkId) {
        ...
        var totals = 0;
        for (let i of this.$store.state.allproblemslist) {
          if (i.score) {
            totals += parseInt(i.score);
          } else if (i.questionScore) {
            totals += parseInt(i.questionScore);
          }
        }
        let path;
        if (this.publishType === "teacher") {
          path = "/homeCommon/TeachingResearch/addHomeWork/release";
        } else {
