<template>
  <div v-if="showNedi">
    ...
    margin-top: -29px;display: block;">{{ item.username }}</span>
          ...
          >
            <el-checkbox v-model="dataList[index].checked" @click="getStuId(item)"></el-checkbox>
          </span>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    //获取班级所有学员
    getClass() {
      this.$post("/user/students/query", { id: this.classId }).then(res => {
        if (res.code == "5000") {
    ...
    edit() {
      ...
      this.$post("/user/group/update", data).then(res => {
        if (res.code == "5000") {
          this.$message({ type: "success", message: "调整成功" });
          this.$parent.chatStudentList();
        } else {
