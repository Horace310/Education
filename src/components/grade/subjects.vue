        ...
        >
          ...
          <span class="custom__tree__icons">
            ...
            <el-tooltip
              content="添加"
              placement="top"
              v-if="data.sectionIndex&& roleId != 2 && !addShow"
            >
  ...
  methods: {
    changeButtoms(val) {
      this.$prompt("请输入需要编辑单元的名称", "提示", {
        ...
        this.$post("/user/resear/updateNode", data).then(res => {
          if (res.code == "5000") {
    ...
    editButtoms(data) {
      this.$confirm("确定删除该单元, 是否继续?", "提示", {
        ...
        this.$deletes("/user/resear/deleteNode", params).then(res => {
          if (res.code == "5000") {
    ...
    addButtoms(data) {
      ...
      this.$prompt("请输入需要添加单元的名称", "提示", {
        ...
        .then(({ value }) => {
          ...
          this.$post("/user/resear/addNode", data).then(res => {
            if (res.code == "5000") {
    ...
    queryTestBookDetailUser(e) {
      ...
      this.$axios({
        url: "http://data.ikeshen.com/user/resear/detail",
        methods: "GET",
        params: gradeId
      }).then(response => {
        if (response.data.code == "5000") {
